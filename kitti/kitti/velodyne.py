...
from kitti.data import get_velo_dir, Calib, get_inds, image_shape, get_calib_dir
...
def load_velodyne_points(drive, **kwargs):
    velodyne_dir = get_velo_dir(drive, **kwargs)
    # points_path = os.path.join(velodyne_dir, "%010d.bin" % frame)
    points_path = velodyne_dir
    points = np.fromfile(points_path, dtype=np.float32).reshape(-1, 4)
    points = points[:, :3]  # exclude luminance
    return points


def load_disparity_points(drive, color=False, **kwargs):

    calib = Calib(color=color, **kwargs)

    # read velodyne points
    points = load_velodyne_points(drive, **kwargs)
...
def lstsq_interp(shape, xyd, lamb=1, maxiter=None, valid=True):
    ...
    if valid:
        ...
        j, i, d = xyd.T
        i0, i1, j0, j1 = int(i.min()), int(i.max()), int(j.min()), int(j.max())
